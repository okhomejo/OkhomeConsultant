...
public final class CalligraphyUtils {
    ...
    //국가 확인 하고 국가 확인따라 scale변경경
    public static void applyCustomFontScale(final Context context, final AttributeSet attr, final TextView textView) {
        //일단 한글인지 영어인지 체크.
        Locale systemLocale = context.getResources().getConfiguration().locale;
        String currentLang = systemLocale.getLanguage(); // ko
        float scaleX = 0f, scaleY = 0f;
        if(currentLang.equals("ko")){
            //한글일경우
            scaleX = resolveFontScale(context, attr, R.attr.fontScaleXKo);
            scaleY = resolveFontScale(context, attr, R.attr.fontScaleYKo);

        }else{
            //알파벳일 경우 혹은 나머지
            scaleX = resolveFontScale(context, attr, R.attr.fontScaleXEn);
            scaleY = resolveFontScale(context, attr, R.attr.fontScaleYEn);
        }
    ...
    //폰트 스케일 가져오기
    private static float resolveFontScale(Context context, AttributeSet attrs, int scaleId) {
        int[] scaleIds = new int[]{scaleId};
        String value = CalligraphyUtils.pullFontPathFromView(context, attrs, scaleIds);

        // Try view style attributes
        if (TextUtils.isEmpty(value)) {
            value = CalligraphyUtils.pullFontPathFromStyle(context, attrs, scaleIds);
        }

        // Try View TextAppearance
        if (TextUtils.isEmpty(value)) {
            value = CalligraphyUtils.pullFontPathFromTextAppearance(context, attrs, scaleIds);
        }
